...
let GalleryComponent = class GalleryComponent {
    ...
    getAllImages() {
        return this.imageService.getAllImages().subscribe(images => {
            this.toolsImageList = images.filter(image => image.Category == 'tool');
            this.projectImageList = images.filter(image => image.Category == 'project');
        });
    }
    ;
    ngOnChanges() {
...
let RequestQouteComponent = class RequestQouteComponent {
    constructor(toastr, fb, emailService) {
        this.toastr = toastr;
        this.fb = fb;
        this.emailPattern = "^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$";
        ...
        this.hasError = (controlName, errorName) => {
            return this.RequestQouteForm.controls[controlName].hasError(errorName);
        };
    }
    ngOnInit() {
        ...
        this.RequestQouteForm.valueChanges.subscribe((data) => {
            this.logValidationErrors(this.RequestQouteForm);
        });
    }
    logValidationErrors(group = this.RequestQouteForm) {
        Object.keys(group.controls).forEach((key) => {
            const abstractControl = group.get(key);
            this.formErrors[key] = '';
            if (abstractControl && !abstractControl.valid
...
RequestQouteComponent.ctorParameters = () => [
    { type: ngx_toastr__WEBPACK_IMPORTED_MODULE_1__["ToastrService"] },
    { type: _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormBuilder"] },
    { type: _shared_services_email_service_email_service__WEBPACK_IMPORTED_MODULE_3__["EmailService"] }
];
